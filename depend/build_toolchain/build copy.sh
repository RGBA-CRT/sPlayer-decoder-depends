...
function build_binutils(){
    ...
    $SRC/${BINUTILS_RELEASE}/configure --prefix=$DEST --with-sysroot=$DEST --target=i686-w64-mingw32 --enable-targets=i686-w64-mingw32,x86_64-w64-mingw32 --disable-werror #--enable-64-bit-bfd
    make -j
    make install
}
...
function build_gcc(){
    ...
    cd $BUILDROOT/gcc
    $SRC/${GCC_RELEASE}/configure --target=i686-w64-mingw32 --prefix=$DEST --with-sysroot=$DEST --enable-targets=all --enable-languages=c,c++,lto --disable-sjlj-exceptions --with-dwarf2 --disable-multilib --enable-libstdcxx-time=yes --enable-threads=win32 --enable-libstdcxx-threads=yes --disable-libgomp --enable-libatomic --enable-lto --enable-fully-dynamic-string 	--disable-libstdcxx-pch	--disable-libstdcxx-debug 	--enable-libstdcxx --disable-rpath	--disable-win32-registry --disable-nls --disable-werror --disable-symvers --with-pkgversion="i686-w64-mingw32, with i486 stdlib" CC="ccache cc" CXX="ccache c++" 
...
function build_crt(){
    ...
    cd $BUILDROOT/crt
    $SRC/${MINGW_RELEASE}/configure --host=i686-w64-mingw32 --prefix=$DEST/i686-w64-mingw32 --with-sysroot=$DEST/i686-w64-mingw32 --enable-lib32 --enable-warnings=0 CC="ccache i686-w64-mingw32-gcc" CXX="ccache i686-w64-mingw32-g++" CFLAGS="-march=i486 -mtune=pentium"
    make -j
    make install
}
...
function gen_link(){
    ...
    for item in "${commands[@]}" ; do
        echo "[ ${item} ]"
        ln $DEST/bin/i686-w64-mingw32-$item $LINK_DIR/i486-i686-w64-mingw32-$item
    done
